...
void loop() {
  ...
  if (r != 0) {
    ...
    else if (r == 5) maze_solve(1);
    else if (r == 6) cal();
    else if (r == 7) while (digitalRead(sw)) motor(10 * spl, 10 * spr);
    else if (r == 8) while (digitalRead(sw));// to test the value of sensor in serial monitor
        else if (r == 9) for (int i = 0; i <= EEPROM.read(19); i++) Serial.println(EEPROM.read(i + 20));
        else if (r == 10) {
          motor(-100, 100); delay(return_delay);
          motor(100, -100); delay(tbr); motor(0, 0);
        }
    motor(0, 0); while (!digitalRead(sw));
    }
  if (bt.available() > 0)bluetooth_control();
}

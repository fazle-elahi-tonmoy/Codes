...
start:    reading();
    if (sum == 0) {
      if (flag != 's') {
        if (scan[counter] == 7) {
          counter++;
          if (scan[counter] == 4 || scan[counter] == 5) grab(scan[counter] - 3);
          else if (scan[counter] == 6) {
            release(); goto start;
          }
        }
        ...
        delay(turn_brake);
        flag = 's'; cross = 's'; pos = 0; t_section = 0;//when you are done turning robot, make the flag to its normal state so that robot wont turn on its own when it finds no line without detecting 90degree logic
      }
      else if (pos < 4 && pos > -4) {
        m1 = millis();
        while (sum == 0) {
      ...
      if (cross != 's') {
        ...
        delay(turn_brake); cross = 's'; flag = 's'; pos = 0; t_section = 0;
      }
      if (sensor == 0b001100) {
        if (pos != 0) {
      ...
      if (s[0] && (s[2] || s[3]) && !s[5]) {
        ...
        if (!s[5]) {
          ...
          if (sum) {
            ...
            if (scan[counter - 1] == 2 && (scan[counter] == 4 || scan[counter] == 5)) grab(scan[counter] - 3);
          }
        }
      }
      ...
      (scan[counter] == 1) ? flag = 'l' : flag = 'r'; t_section = 1; 
      delay(node_delay); reading();
      if (sum != 0) {
        t_section = 0;
        if (scan[counter] == 1) cross = 'l' ;

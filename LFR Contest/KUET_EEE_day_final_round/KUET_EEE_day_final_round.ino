...
#define trl 24
#define ecl 26
int sf = 0, sl = 0, sr = 0, obs = 0, midpoint = 11;
...
#define switchin 51
#define switchout 53
#define buzzer_positive 33
#define buzzer_negative 39
...
void cal() {
  ...
  motorSpeed(0, 0);
  delay(500);
  for (int i = 0; i < 6; i++) trash[i] = ( maximum[i] + minimum[i]) * 0.5;
  for (int i = 0; i < 6; i++) {
  ...
  while (r == 0) {
    ...
    if (r != 0) {
      if (r == 1) {
        ...
        while (digitalRead(calin) == LOW) laser_reading_display();
      }
      sust_cracker_nut();
      delay(500);
    }

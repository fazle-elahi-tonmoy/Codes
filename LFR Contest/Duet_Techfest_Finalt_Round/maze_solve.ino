...
a:    reading();
    if (sum == 0) {
      ...
      if (s[0] == 0 && s[5] == 1 && s[2] + s[3] > 0) {
        flag = 'l';
        if (scan[counter] == 1 || scan[counter] == 2) {
        ...
        else if (scan[counter] == 3) {
          while (s[0] == 1 && s[5] == 0 && s[2] + s[3] > 0) reading();
          while (sum == 0) reading();
          while (sum) reading();
          delay(node_delay); braking(); digitalWrite(head_light, HIGH);
          motor(-100, 100); delay(return_delay);
          motor(100, -100); delay(tbr); motor(10 * spl, 10 * spr);
          counter++; pos = 0; flag = 's'; cross = 0;  digitalWrite(head_light, LOW);
        }
        else if (scan[counter] == 5) {
          m3 = millis();

...
byte scan[50] = {1, 1, 1, 4, 2, 1, 2, 2, 5};
byte counter = 0;
byte cross = 0;
...
void loop() {
  ...
  if (r != 0) {
    ...
    else if (r == 6) for (int i = 0; i <= EEPROM.read(19); i++) Serial.println(EEPROM.read(i + 20));
    else if (r == 7) counter = 2;
    else if (r == 8) counter = 4;
    motor(0, 0); digitalWrite(head_light, 0); while (!digitalRead(sw));
  }

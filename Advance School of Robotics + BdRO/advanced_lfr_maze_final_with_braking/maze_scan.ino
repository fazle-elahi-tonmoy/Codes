...
a:    reading();
    if (sum == 0) {
      ...
      while (s[0] == 1 || s[5] == 1) {
        reading(); m2 = millis();
        if (m2 - m3 > stop_timer) {
          motor(-250, -250); delay(cl/brake_constant); motor(0, 0);
          scan[counter] = 5; algorithm();
          if (initial == 1) {
            motor(-150, 150); delay(return_delay);
            motor(150, -150); delay(tbr);
            maze_solve(1);
          }
          return;
        }

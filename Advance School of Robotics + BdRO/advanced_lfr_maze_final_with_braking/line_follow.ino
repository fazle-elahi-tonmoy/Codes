...
a:    reading();
    if (sum == 0) {
      ...
      if (cross != 0) {
        ...
        motor(10 * spl, 10 * spr); if (cl < peak) cl++;
      }
      //right side portion
      else if (sensor == 0b000100) {
        motor(10 * spl, 9 * spr); if (cl < peak) cl++;
      }
        ...
        if (pos < 4) pos = 4; motor(10 * spl, -3 * spr); if (cl > strt) cl--;
      }
      //left side portion
      else if (sensor == 0b001000) {
        motor(9 * spl, 10 * spr); if (cl < peak) cl++;
      }
        ...
        if (pos > -4) pos = -4; motor(-3 * spl, 10 * spr); if (cl > strt) cl--;
      }
    }
      ...
      m3 = millis(); flag = 'r'; cross = 0; pos = 0;
      while (s[0] == 1 || s[5] == 1) {
        reading(); m2 = millis();
        if (m2 - m3 > stop_timer) {
          motor(-250, -250); delay(cl / brake_constant); motor(0, 0); digitalWrite(12, HIGH);

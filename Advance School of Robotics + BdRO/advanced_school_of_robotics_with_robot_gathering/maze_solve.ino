void maze_solve(bool logic) {
  memory_recall(); counter = 0;
  if (logic) {
    while (1) {
      if (scan[counter] == 1) scan[counter] = 3;
      else if (scan[counter] == 3) scan[counter] = 1;
      else if (scan[counter] == 5) break;
      counter++;
    }
    counter--;
  }
      ...
      if (s[0] && (s[2] || s[3]) && !s[5]) {
        ...
        if (!s[5]) {
          ...
          if (sum) {
            ...
            (logic) ? counter-- : counter++; digitalWrite(led, 1);
          }
        }
      }
